#!/usr/bin/env python
"""
Test (infer with) an Exact GP model on a spatio-temporal dataset using GPyTorch.
...
def main():
    parser = argparse.ArgumentParser(description='Spatio-temporal Exact GP Testing Script')
    parser.add_argument('--checkpoints', type=str, nargs='+', required=True,
                        help='Paths to one or more checkpoint files to compare.')
    parser.add_argument('--results_folder', type=str, default='results',
                        help='Folder to store PDF results and final metrics plots.')
    ...
    parser.add_argument('--test_mask_type', type=str, choices=['fixed', 'dynamic'], default=None,
                        help='Override for mask type at test time.')
    parser.add_argument('--test_prob_masking', type=float, default=None,
                        help='Override for masking probability at test time.')
    ...
    for checkpoint_path in args.checkpoints:
        ...
        # Extract saved training parameters
        ...
        print(f"From checkpoint: time_kernel={time_kernel}, space_kernel={space_kernel}, "
              ...
              f"testFrames={num_frames_test}.")

        # Load data up to train+val+test frames
        ...
        # Prepare original training/validation data
        ...
        # Prepare test data with possible override
        ...
        with PdfPages(pdf_filename) as pdf:
            # Train frames
            train_mesh = generate_meshgrid(num_frames_train, height=17, width=48, start_time_index=0)
            train_coords = np.stack(train_mesh, axis=-1).reshape(-1, 3)
            train_coords_torch = torch.from_numpy(train_coords).float().to(device)
            train_mean, train_var = run_inference(model, likelihood, train_coords_torch,
                                                  num_frames=num_frames_train,
                                                  height=17, width=48)
            ...
            # Validation frames
            val_mesh = generate_meshgrid(num_frames_val, height=17, width=48,
                                         start_time_index=num_frames_train)
            val_coords = np.stack(val_mesh, axis=-1).reshape(-1, 3)
            val_coords_torch = torch.from_numpy(val_coords).float().to(device)
            val_mean, val_var = run_inference(model, likelihood, val_coords_torch,
                                              num_frames=num_frames_val,
                                              height=17, width=48)
            ...
            # Test frames
            test_mesh = generate_meshgrid(num_frames_test, height=17, width=48,
                                          start_time_index=num_frames_train + num_frames_val)
            test_coords = np.stack(test_mesh, axis=-1).reshape(-1, 3)
            test_coords_torch = torch.from_numpy(test_coords).float().to(device)
            test_mean, test_var = run_inference(model, likelihood, test_coords_torch,
                                                num_frames=num_frames_test,
                                                height=17, width=48)
    ...
    ax1.set_title("MAE and RMSE across Kernels (Exact GP)")
    ax1.set_xticks(x)
    ...
    print("\nDone comparing Exact GP checkpoints!")
